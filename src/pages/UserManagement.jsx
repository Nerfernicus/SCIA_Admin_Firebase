...
import {
    Search, Bell, Settings, Users,
    ShieldAlert, Ban, SlidersHorizontal,
    RotateCcw, CheckCircle2,
    ArrowRight, Trash2, Download, X, ChevronDown,
    ThumbsUp, ThumbsDown
} from 'lucide-react';
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 z-10 text-center">
                ...
                <h2 className="text-lg font-bold text-gray-900 mb-1">Disapprove User?</h2>
                <p className="text-sm text-gray-500 mb-1">You are about to disapprove</p>
...
export default function UserManagement() {
    ...
    const handleDisapproveConfirm = async () => {
        ...
        const name = `${disapproveTarget.firstName ?? ''} ${disapproveTarget.lastName ?? ''}`.trim();
        await deleteDoc(doc(db, 'users', disapproveTarget.id));
        setSelected(prev => { const n = new Set(prev); n.delete(disapproveTarget.id); return n; });
    ...
    const handleMassDeactivate = async () => {
        if (selected.size === 0) { showToast('Select at least one user first.'); return; }
    ...
    const handleDownloadCSV = () => {
        const rows = filtered.map(u => ({
            ...
            Registered: u.createdAt?.toDate ? u.createdAt.toDate().toLocaleDateString() : '',
        }));
        if (rows.length === 0) { showToast('No users to export.'); return; }
    ...
    const filterLabels = { ALL: 'All Users', ACTIVE: 'Active', PENDING: 'Pending', SUSPENDED: 'Suspended' };
    ...
    return (
        <div className="flex-1 bg-[#f8f9fa] min-h-screen p-8 font-sans relative">
            ...
            {/* Toast Notification */}
            {toastMsg && (
                <div className={`fixed top-6 right-6 z-[9999] text-white text-sm font-medium px-5 py-3 rounded-2xl shadow-xl flex items-center gap-3 animate-fade-in ${
                    ...
                    {toastMsg}
                    <button onClick={() => setToastMsg('')}><X size={14} className="text-white/60 hover:text-white" /></button>
                </div>
            )}
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-4 text-gray-500">
                    <button className="hover:text-gray-800 relative">
                        <Bell size={20} />
                        {pendingCount > 0 && (
                    ...
                    <Link to="/settings" className="hover:text-gray-800 transition-colors">
                        <Settings size={20} />
                    </Link>
            ...
            {/* Table */}
            <div ref={tableRef} className="bg-white rounded-2xl shadow-sm border border-gray-100 mb-8 overflow-hidden">
                            ...
                            <div className="absolute right-0 top-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 py-2 w-48">
                                {Object.entries(filterLabels).map(([val, label]) => (
                                    <button key={val}
                                        ...
                                        <span className={`w-1.5 h-1.5 rounded-full ${statusFilter === val ? 'bg-[#0f52ba]' : 'bg-transparent'}`}></span>
                                        {label}
                                    </button>
                                ))}
                            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {loading && (
                                <tr><td colSpan={7} className="py-10 text-center text-gray-400 text-sm">Loading users...</td></tr>
                            )}
                                    ...
                                    <tr key={user.id}
                                        ...
                                        <td className="py-4 px-6">
                                            <p className="font-bold text-gray-900 text-sm">
                                                {user.firstName} {user.midName ? user.midName[0] + '.' : ''} {user.lastName}
                                            </p>
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-2">
                                                <span className={`w-2 h-2 rounded-full ${statusDot(user.status ?? 'PENDING')}`}></span>
                                                <span className={`text-xs font-bold ${statusText(user.status ?? 'PENDING')}`}>
                                                    {user.status ?? 'PENDING'}
                                                </span>
                                            </div>
                                        </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-[#f4f6fc] rounded-2xl p-6 border border-blue-50">
                    ...
                    <p className="text-sm text-gray-500 mb-6">
                        {selected.size > 0 ? `${selected.size} user(s) selected.` : 'Select users from the table above using checkboxes.'}
                    </p>
                    <div className="flex flex-wrap gap-3">
                        ...
                        <button onClick={handleMassDeactivate}
                            ...
                            <Ban size={15} /> Mass Deactivate {selected.size > 0 ? `(${selected.size})` : ''}
                        </button>
                    </div>
                </div>
